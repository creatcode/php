...
final class Loader {
	...
	public function view($route, $data = array()) {
        $data = is_array($data) ? array_merge($this->common, $data) : array();
		// Sanitize the call
	...
	public function library($route, $used_db = true) {
		// Sanitize the call
		$route = preg_replace('/[^a-zA-Z0-9_\/]/', '', (string)$route);
...
//			$this->registry->set(basename($route), new $class($this->registry));
            if ($used_db) {
                $this->registry->set(str_replace(array('/', '-', '.'), array('_', '', ''), (string) $route), new $class($this->registry));
            } else {
                $this->registry->set(str_replace(array('/', '-', '.'), array('_', '', ''), (string) $route), new $class);
            }
		} else {
    ...
    {
        if (in_array(strtolower($method), array('library_logic', 'sys_model', 'tool'), true)) {
