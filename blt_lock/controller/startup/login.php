...
class ControllerStartupLogin extends Controller {
    public function index() {
        ...
        if (!in_array($route, $ignore)) {
            ...
            #验证token
            $this->load->library('logic/blt_lock', true);
            $user_name  = $this->request->post['user_name'];
            $sign       = $this->request->post['sign'];
            #解密处理；

            $result     = $this->logic_blt_lock->checkUserSign(array('user_name' => $user_name), $sign);
            if (!$result['state']) {
...
//            }
            ...
            $this->registry->set('startup_user', $this->logic_blt_lock);
        }
    }
}