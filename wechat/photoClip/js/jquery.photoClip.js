...
(function(root, factory) {
	...
	$.fn.photoClip = function(option) {
		if (!window.FileReader) {
			mui.alert("您的浏览器不支持 HTML5 的 FileReader API， 因此无法初始化图片裁剪插件，请更换最新的浏览器！",'温馨提示');
			return;
		}
	...
	function photoClip(container, option) {
		...
		$file.change(function() {
			...
			if (!/image\/\w+/.test(this.files[0].type)) {
				mui.alert("图片格式不正确，请选择正确格式的图片文件！",'温馨提示');
				return false;
			} else {
				var fileC=this.files[0];
				EXIF.getData(fileC, function(){
					...
					fileReader.onload = function(e) {
						...
						//输出图片
						getImgData(this.result, orientation,800, function(data) {
							//这里可以使用校正后的图片data了
					...
					fileReader.onerror = function(e) {
						mui.alert("图片加载失败",'温馨提示');
						loadError.call(this, e);
					};
					fileReader.readAsDataURL(fileC); // 读取文件内容
	
					loadStart.call(fileReader, fileC);
				});
