...
class ControllerOperationIntelligent extends Controller {
	...
	protected function getDataColumns() {
        ...
        $this->setDataColumn('定位次数(次)');
		$this->setDataColumn('定位时长(分钟)');
	...
	public function repeatdata(){
		...
		if(isset($filter['type'])){//data_type 是判断读什么数据的依据,默认是单车
			switch($filter['type']){
				case 'bicycle':$data_type='bicycle';break;
				case 'lock':$data_type='lock';break;
				default :$data_type='bicycle';break;
			}
		}else{
			$data_type='bicycle';
		}
		...
		switch($data_type){
			case 'bicycle':
				$data_columns=array('单车编号','车锁编号','单车类型','锁类型','城市','合伙人','是否使用中');
				...
				break;
			case 'lock':
				$data_columns=array('锁编号','锁名称','合伙人','当前电量(百分比)','开锁次数','更新时间','状态');
