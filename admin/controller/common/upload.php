...
class ControllerCommonUpload extends Controller {
    ...
    public function index() {
        ...
        $useOriName = $this->request->post('useOriName') ? true : false;
        $config = array(
            'pathFormat'=>$pathFormat,
            "maxSize" => $maxSize,
            "allowFiles" => $allowFiles,
            'useOriName' => $useOriName
        );
        // 文件变量名称
        $fileField = 'upfile';
        // 文件类型
        $type = $this->request->post('fileType') ? $this->request->post('fileType') : 'upload';
