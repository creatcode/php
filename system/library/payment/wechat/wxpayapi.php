...
class WxPayApi
{
	 ...
	 */
	public static function getCallClientStr($inputObj) {
		//检测必填参数
		if(!$inputObj->IsPrepayidSet()) {
			throw new WxPayException("缺少调起支付接口必填参数prepayid！");
		}
		// 暂填写固定值Sign=WXPay
		if (!$inputObj->IsPackageSet()) {
			$inputObj->SetPackage('Sign=WXPay');
		}
		...
		$result = WxPayResults::InitFromArray($inputObj->GetValues(),true)->GetValues();
