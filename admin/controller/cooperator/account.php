...
class ControllerCooperatorAccount extends Controller {
    ...
    public function index() {
        ...
        $filter = $this->request->get(array('cooperator_id', 'admin_name', 'nickname'));
        ...
        if (!empty($filter['nickname'])) {
            $condition['nickname'] = array('like', "%{$filter['nickname']}%");
        }
	...
	$this->assign('import_action', $this->url->link('cooperator/account/import_action'));
	$this->assign('export_action', $this->url->link('cooperator/account/export_action'));
    ...
    public function add() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $cooperator_id = $this->request->get('cooperator_id');
            $input = $this->request->post(array('admin_name', 'password', 'state', 'role_id', 'nickname'));
            $now = time();
            $data = array(
            ...
            $filter = $this->request->get(array('admin_name', 'nickname', 'login_time', 'role_id', 'state'));
...
//        }
	...
	if (!$this->request->get('admin_id') && empty($this->request->post['admin_name'])) {
            $this->error['admin_name'] = '请输入管理员名称';
        }
        if (!$this->request->get('admin_id') && empty($this->request->post['nickname'])) {
            $this->error['nickname'] = '请输入姓名';
        }
	if (empty($this->request->post['role_id'])) {
            $this->error['role_id'] = '请选择角色';
        }
        if (!$this->request->get('admin_id') && empty($password)) {
            $this->error['password'] = '请输入密码';
        }
        if (!empty($password)) {
            if ($password !== $confirm) {
