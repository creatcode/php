<?php
class ControllerCooperatorRole extends Controller {
    ...
    public function index() {
        ...
        if (is_array($result) && !empty($result)) {
            foreach ($result as &$item) {
                ...
                $item['edit_action'] = $this->url->link('cooperator/role/edit', 'role_id='.$item['role_id'] . '&cooperator_id='.$item['cooperator_id']);
                $item['delete_action'] = $this->url->link('cooperator/role/delete', 'role_id='.$item['role_id'] . '&cooperator_id='.$item['cooperator_id']);
            }
        }
        ...
        $this->assign('add_action', $this->url->link('cooperator/role/add', 'cooperator_id='.$cooperator_id));
        ...
        $this->assign('update_admin_region_action', $this->url->link('cooperator/role/update_admin_region'));
        ...
        $pagination->url = $this->cur_url . '&amp;page={page}' . '&amp;cooperator_id=' . $cooperator_id;
        ...
        $this->response->setOutput($this->load->view('cooperator/role_list', $this->output));
    }
    ...
    public function add() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            ...
            $this->load->controller('common/base/redirect', $this->url->link('cooperator/role') . '&' . http_build_query($filter) . '&cooperator_id=' . $cooperator_id);
        }
    ...
    public function info() {
        ...
        $this->response->setOutput($this->load->view('cooperator/role_info', $this->output));
    }
    ...
    private function getForm() {
        ...
        $this->assign('data', $info);
        $this->assign('action', $this->cur_url . '&role_id=' . $role_id . '&cooperator_id=' . $cooperator_id);
        $this->assign('return_action', $this->url->link('cooperator/role', 'cooperator_id=' . $cooperator_id));
        $this->assign('error', $this->error);
        $this->assign('static', HTTP_IMAGE);

        $this->response->setOutput($this->load->view('cooperator/role_form', $this->output));
    }
