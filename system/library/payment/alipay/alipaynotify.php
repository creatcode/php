...
library('payment/alipay/alipayTool');
/* *
 * 类名：AlipayNotify
 * 功能：支付宝通知处理类
 * 详细：处理支付宝各接口通知返回
...
class alipayNotify {
    ...
    public function AlipayNotify($alipay_config = array()) {
	...
	public function getSignVerify($para_temp, $sign) {
		...
		$pre_str = aliPayTool::createLinkString($para_sort);
		
		$isSign = false;
		switch (strtoupper(trim($this->alipay_config['sign_type']))) {
			case "RSA" :
				$isSign = aliPayTool::rsaVerify($pre_str, trim($this->alipay_config['ali_public_key_path']), $sign);
				break;
			case "MD5" :
				$isSign = md5Verify($pre_str, $sign, $this->alipay_config['key']);
				break;
			default :
				$isSign = false;
		}
	...
	public function getResponse($notify_id) {
		...
		$responseTxt = aliPayTool::getHttpResponseGET($verify_url, $this->alipay_config['cacert']);
