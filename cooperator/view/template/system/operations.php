...
class ControllerSystemOperations extends Controller {
    ...
    public function add() {
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
            $input = $this->request->post(array('admin_name', 'nickname', 'password', 'state', 'role_id','mobile'));
            $this->load->library('sys_model/rbac', true);
            if(isset($input['role_id']) && $input['role_id'] != ''){
                $w['role_id'] = $input['role_id'];
                $result = $this->sys_model_rbac->getRoleInfo($w);
            }
            $now = time();
            $data = array(
                ...
                'mobile'         => $input['mobile'],
                'type'           => 3,
                'cooperator_id' => isset($result['cooperator_id']) ? $result['cooperator_id'] : 0,
    ...
    private function validateForm() {
        ...
        if(empty($admin_name)){
            $this->error['admin_name'] = '账号不能为空！';
        }
        if(empty($cooperator_id)){
            $this->error['cooperator_id'] = '账号归属不能为空！';
        }
        # 判断该账号是否已经添加过；
        $result = $this->sys_model_admin->getAdminInfo(array("admin_name" => $admin_name));
        if(!empty($result)){
            $this->error['admin_name'] = '账号已经存在，不能添加！';
        }
        if ($this->error) {
