...
//        $this->load->library('sys_model/repair', true);
    ...
    {
        $filter = $this->request->get(array('add_time', 'admin_name'));
        //获取合伙人
        $condition = [];
        $admin_condition = ['type' => 3, 'state' => 1 ,'cooperator_id'=>$this->cooperator_id];
        ...
        $cooperator_condition['cooperator_id'] = $this->cooperator_id;
		...
		if (!empty($filter['add_time'])) { 
            ...
            $condition['add_time'] = [
                ['egt', strtotime($add_time[0])],
                ['elt', bcadd(86399, strtotime($add_time[1]))]
            ];
        }
