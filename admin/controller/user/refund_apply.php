...
class ControllerUserRefundApply  extends Controller {
    ...
    public function index() {
        $filter = $this->request->get(array('apply_city','apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time','city_id','time_type','region_id','apply_payment_type'));
        ...
        $field = 'cash_apply.*, city.city_name,region.region_name';
        $join = array(
        ...
        $total = $this->sys_model_deposit->getTotalCashApply($condition,$join);
        ...
        $this->assign('reginster_list', $this->url->link('user/refund_apply/reginster_list'));
    ...
    public function export() {
        $filter = $this->request->get(array('apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time','city_id','region_id'));
        ...
        $field='*';
        $order = 'apply_id DESC';
        $result = $this->sys_model_deposit->getCashApplyList($condition, $order,"",$field,$join);
        $apply_states = get_apply_states();
        $list = array();
        if (is_array($result) && !empty($result)) {
            foreach ($result as $v) {
    ...
    private function getForm() {
        ...
        $this->assign('return_action', $this->url->link('user/refund_apply'));
    ...
    public function exports() {
        ...
        $join = array(
            ...
            'user' => 'user.user_id=deposit_apply.apply_user_id',
            'city' => 'city.city_id=user.city_id',
            'region' => 'region.region_id=user.region_id'
        );

        $result = $this->sys_model_deposit->getDepositApplyList($condition, $order,$field,$join);
    ...
    public function deposit_list(){
        $filter = $this->request->get(array('apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time', 'apply_payment_type','city_id','time_type','region_id'));
        ...
        $offset = ($page - 1) * $rows;
        $limit = sprintf('%d, %d', $offset, $rows);
        $field = 'deposit_apply.*,pdr_payment_code,pdr_payment_name,pdr_payment_type,city.*,region.region_name';
        $join = array(
            'deposit_recharge'=>'deposit_recharge.pdr_sn=deposit_apply.pdr_sn',
            'user' => 'user.user_id=deposit_apply.apply_user_id',
            'city' => 'city.city_id=user.city_id',
        ...
        $total = $this->sys_model_deposit->getTotalDepositApply($condition,$join);
        ...
        $this->assign('export_action', $this->url->link('user/refund_apply/exports'));
        $this->assign('index_action', $this->url->link('user/refund_apply'));
    ...
    public function export1() {
        ...
        $offset = ($page - 1) * $rows;
        $limit = sprintf('%d, %d', $offset, $rows);
        $field = 'reginster_apply.*,pdr_payment_code,pdr_payment_name,pdr_payment_type,city.*';
        $join = array(
            'deposit_recharge'=>'deposit_recharge.pdr_sn=reginster_apply.pdr_sn',
            'user' => 'user.user_id=reginster_apply.apply_user_id',
            'city' => 'city.city_id=user.city_id',
            'region' => 'region.region_id=user.region_id'
        );

        $result = $this->sys_model_deposit->getReginsterApplyList($condition, $order,$field,$join);
    ...
    /**
     * [deposit_list 获取注册金退款申请列表]
     */
    public function reginster_list(){
       $filter = $this->request->get(array('apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time', 'apply_payment_type','city_id','time_type','region_id'));
        ...
        $offset = ($page - 1) * $rows;
        $limit = sprintf('%d, %d', $offset, $rows);
        $field = 'reginster_apply.*,pdr_payment_code,pdr_payment_name,pdr_payment_type,city.*,region.region_name';
        $join = array(
            'deposit_recharge'=>'deposit_recharge.pdr_sn=reginster_apply.pdr_sn',
            'user' => 'user.user_id=reginster_apply.apply_user_id',
            'city' => 'city.city_id=user.city_id',
        ...
        $result = $this->sys_model_deposit->getReginsterApplyList($condition, $order, $limit, $field, $join);
        $total = $this->sys_model_deposit->getTotalReginsterApply($condition,$join);
        ...
        $apply_states_colors = array(0=>'text-blue', 1=>'text-blue',2=>'text-green', -1=>'text-red');
        // 是否拥有审核权限
        $show_audit_action_tech = $this->logic_admin->hasPermission('user/refund_apply/audit_regin_tech');//技术审批
        $show_audit_action_fina = $this->logic_admin->hasPermission('user/refund_apply/audit_regin_fina');//财务审批
        if (is_array($result) && !empty($result)) {
            foreach ($result as &$item) {
                $item = array(
                    'region_name' => $item['region_name'],
                    ...
                    'apply_audit_result' => strlen($item['apply_audit_result'])>30 ? substr($item['apply_audit_result'],0,30) . '...' : $item['apply_audit_result'],
                    'apply_audit_time' => !empty($item['apply_audit_time']) ? date('Y-m-d<\b\r/>H:i:s', $item['apply_audit_time']) : '',
                    'audit_action_tech' => $show_audit_action_tech && $item['apply_state'] == 0 ? $this->url->link('user/refund_apply/audit_regin_tech', http_build_query($filter) . '&page='. $page . '&apply_id='. $item['apply_id']) : '',
                    'audit_action_fina' => $show_audit_action_fina && $item['apply_state'] == 1 ? $this->url->link('user/refund_apply/audit_regin_fina', http_build_query($filter) . '&page='. $page . '&apply_id='. $item['apply_id']) : '',
                );
            }
        }
        ...
        $this->assign('filter_types', $filter_types);
        $this->assign('action', $this->cur_url);
        $this->assign('add_action', $this->url->link('user/cashapply&pdc_type=2&pdc_payment_state=0'));
        ...
        $this->assign('export_action', $this->url->link('user/refund_apply/export1'));
        $this->assign('index_action', $this->url->link('user/refund_apply'));
        $this->assign('deposit_list', $this->url->link('user/refund_apply/deposit_list'));

        $this->response->setOutput($this->load->view('user/refund_reginster_list', $this->output));
    }
     ...
     * @return   [type]     [description]
     */
    public function audit_regin_tech(){
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            $apply_id = $this->request->get('apply_id');
            $input = $this->request->post(array('apply_state', 'apply_audit_result'));
            ...
            $pda_info   = $this->sys_model_deposit->getReginsterApplyInfo($condition);
            ...
            if ($input['apply_state'] == 1) {// 审核通过
                ...
                $condition1['pdc_payment_state'] = 3;//只能更新申请状态订单
                $data1['pdc_payment_state']      = 0;
                $res1 = $this->sys_model_deposit->updateReginsterApply($condition1, $data1);
                //var_dump($this->db->getLastSql());
            }
            ...
            $res2   = $this->sys_model_deposit->updateReginsterApply($condition2, $data2);
            if($res1 && $res2){
                $this->db->commit();
                $filter = $this->request->get(array('apply_payment_type' ,'apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time', 'page'));
                $this->load->controller('common/base/redirect', html_entity_decode($this->url->link('user/refund_apply/deposit_list', $filter, true)));
                ...
                $this->db->rollback();
                $this->error['warning'] = '审核失败：更新注册金退款记录 :'.$res1.';更新注册金退款申请记录：'.$res2;
                goto showForm;
            }
        }
        showForm:
        $this->assign('title', '注册金退款审核');
        $this->getReginForm();
    }
     ...
     * @DateTime 2017-07-26T16:23:25+0800
     */
    public function audit_regin_fina() {
        if ($this->request->server['REQUEST_METHOD'] == 'POST' && $this->validateForm()) {
            $success = true;
            $apply_id = $this->request->get('apply_id');
            $input = $this->request->post(array('apply_state', 'apply_audit_result'));
            ...
            $res4     = $this->sys_model_deposit->updateReginsterApply($condition4, $data4);
            if($res1 && $res2 && $res3 && $res4){//成功
                $this->db->commit();
                $filter = $this->request->get(array('apply_payment_type' ,'apply_user_name', 'pdr_sn', 'apply_admin_name', 'apply_audit_admin_name', 'apply_state', 'apply_add_time', 'apply_audit_time', 'page'));
                $this->load->controller('common/base/redirect', html_entity_decode($this->url->link('user/refund_apply/deposit_list', $filter, true)));
    ...
    private function getReginForm() {
        ...
        $cash_apply_info = $this->sys_model_deposit->getReginsterApplyInfo($condition);
        ...
        $this->response->setOutput($this->load->view('user/refund_reginster_form', $this->output));
    }
