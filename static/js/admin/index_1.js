...
jQuery(function ($) {
    ...
    window.initMap = function () {
        ...
        map = new google.maps.Map(document.getElementById('map'), {
            center: {lat: 23.002974, lng: 113.73027},
            zoom: 10
        });
        loadMarker(map);
        ...
        google.maps.event.addListener(map, 'zoom_changed', function() {  //zoom改变触发的事件
            
          });  
        google.maps.event.addListener(map, 'bounds_changed', function() {  //当可视区域范围更改时会触发此事件
            //alert('222');
          });  
    ...
    function loadMarker() {
        google.maps.event.addListener(map, 'bounds_changed', function () {
            ...
            var max_lat = bounds.f.f;
            var min_lat = bounds.f.b;
            var max_lng = bounds.b.f;
            var min_lng = bounds.b.b;
            var data = {min_lng: min_lng, min_lat: min_lat, max_lng: max_lng, max_lat: max_lat};
    ...
    function mrkerLoaded(data) {
        ...
        $.each(data.data, function (i, bike) {
            ...
            var marker, pos = {lng: Number(bike.lng), lat: Number(bike.lat)};

            bike.icon = getMarkerIcon(bike);;
            if (!_markers[bike.bicycle_sn]) {

                marker = new google.maps.Marker({
                    map: map,
                    icon: bike.icon,
                    title: bike.bicycle_sn,
                    position: pos,
                    bike: bike
                });
                ...
                //if ((Math.abs(oldPos.getLng() - pos['lng']) > 0.000001) || (Math.abs(oldPos.getLat() - pos['lat']) > 0.000001)) {
            ...
            if (infoWindowOpened == bike.bicycle_id) {
	...
	getMarkerIcon = function (bike) {
    var BASE_URL = window.imageUrlBase + 'images/map_icon/';
	...
	iconObj.url = BASE_URL+'j1.png';
    return iconObj;

    if (bike.offline == '1') {
        iconObj.url = BASE_URL + 'home_map_ic3@3x.png';
        return iconObj;
    } else if(bike.faultTime && bike.illegalParkingTime) {
        (bike.faultTime < bike.illegalParkingTime) ? iconObj.url = BASE_URL + 'home_map_ic03@3x.png' : iconObj.url = BASE_URL + 'home_map_ic02@3x.png';
        return iconObj;
    }
    else if(bike.faultTime) {
        iconObj.url = BASE_URL + 'home_map_ic02@3x.png';
        return iconObj;
    }
    else if(bike.illegalParkingTime) {
        iconObj.url = BASE_URL + 'home_map_ic03@3x.png';
        return iconObj;
    }
    else if(Math.abs(parseInt(bike.battery))<=25) {
        iconObj.url = BASE_URL + 'home_map_i01@3x.png';
        return iconObj;
    }
    else if(bike.noUsedDays >= 2 && bike.noUsedDays <= 5) {
        iconObj.url = BASE_URL + 'home_map_ic2' + bike.noUsedDays + '.png';
        return iconObj;
    }
    else if(bike.noUsedDays > 5) {
        iconObj.url = BASE_URL + 'home_map_ic26.png';
        return iconObj;
    }
    else if(bike.is_using=="1") {
        iconObj.url = BASE_URL + 'home_map_ic2@3x.png';
        return iconObj;
    }
    else {
        iconObj.url = 'home_map_ic1@3x.png';
        return iconObj;
    }
};
    ...
    function createInfoWindow() {
        ...
        $bikeInfo = $(
                ...
                '<div class="bike-info">' +
                '<div class="bike-info-title">车辆编号：<span class="bike-sn">123456</span></div>' +
                ...
                '<hr>' +
                '<div class="bike-lock-sn font14"><i class="fa fa-lock fa-fw"></i> RFID：<span>063012345678</span> <i class="fa fa-info-circle bike-lock-info"></i></div>' +
               ...
               */ '</div>' + // 指令 end
				...
				'<div class="bike-info-close"><i class="fa fa-close"></i></div>' +
                '</div>'
                );
    ...
    var bike = self.bike;
    updateInfoWindow(bike);
    infoWindow.setContent($bikeInfo[0]);
    ...
    function search() {
        ...
        $.ajax('index.php?route=admin/index/search', {
            ...
            success: function (result) {
                ...
                if (html === '')
                    html = '<div style="padding: 10px;">没有相关单车或用户</div>';
                $('.bike-list').html(html);
            }
        });
    }
    ...
    function updateInfoWindow(bike) {
		...
		geoCoder.geocode({'location': {lat: pos[1], lng: pos[0]}}, function (results, status) {
        if (status === 'OK') {
            if (results[1]) {
                $bikeInfo.find('.bike-location').html('<i class="fa fa-map-marker fa-fw"></i> <span>'+ results[1].formatted_address +'</span> ');
            }
        }
    });
        // 二维码
    ...
    function refreshTab() {
        ...
        if (index > 0 && index < 5)
            $tabDiv.find('ul').empty(); //0和5是不清空内容的，只更新内容
    ...
    $("#map-toolbar .region-selecter").change(function () {
        ...
        var address_name=$("#map-toolbar .region-selecter").find("option:selected").text();
        $.get("https://maps.googleapis.com/maps/api/geocode/json?address="+address_name+"&key=AIzaSyDunT2WYI4g1z--h5nm6yInWa0qlBgqR2Q",{},function(data){
           map.setCenter(data.results[0].geometry.location);//假设都找到对应坐标，没找到的后面再补锅了
    ...
    function userInfoWindow(user_id) {
        userInfoWindowModel = $(
                ...
                '<table class="table table-striped table-hover" style="margin-bottom: 0px;">' +
                '<thead><tr><th>车辆</th><th>区域</th><th>城市</th><th>金额</th><th>结算时间</th><th>优惠券</th><th>状态</th></tr></thead>' +
                '<tbody>' +
                '</tbody>' +
        ...
        $.ajax('index.php?route=admin/index/userInfo', {
            ...
            success: function (result) {
                ...
                var userInfo =
                        ...
                        '</div>' +*/
                        '<div class="form-group col-sm-6">' +
                        '<label for="" class="col-sm-5 control-label">邮箱地址：</label>' +
                        '<div class="col-sm-7">' +
                        '<span>' + '123@outlook.com' + '</span>' +
                        '</div>' +
                        '</div>' +
                        '<div class="form-group col-sm-6">' +
                        '<label for="" class="col-sm-5 control-label">facebook 名称：</label>' +
                        '<div class="col-sm-7">' +
                        '<span>' + '喵喵喵' + '</span>' +
                        '</div>' +
                        '</div>'+
                        ...
                        '<select onchange="change_available_state(this)" data-user_id="'+result.data.user_id+'"><option value="1">是</option><option value="0">否</option></select>'+
                        '</div>' +
                        '</div>' +
                        '<div class="form-group col-sm-6">' +
...
function change_available_state(t){
        var user_id=$(t).attr('data-user_id');
        var state=$(t).val();
        $.post("index.php?route=admin/index/change_available_state",{"user_id":user_id,'state':state},function(){
