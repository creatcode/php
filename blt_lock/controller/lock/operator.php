 ...
 * Time: 16:03
 */
class ControllerLockOperator extends Controller
{
    /**
     * 开锁
     */
...
//            $full_bicycle_sn = $region_info['region_city_code'].str_pad($region_info['region_sort'],2,0,STR_PAD_LEFT).trim($this->request->post['bicycle_sn']);
        ...
        $data = ['mac_address' => $bike_loke_info[0]['mac_address']];

        # 开锁
        if (in_array($bike_loke_info[0]['lock_type'], array(2, 5))) {
            $other_info = array(
        ...
        $this->response->showSuccessResult($data, '开锁指令已发');
    ...
    {
        $input = $this->request->post(array('mac_address', 'encrypt_key'));
        ...
        if ($lock_info['lock_factory'] == 2) {
            if (empty($input['encrypt_key']) || (strlen($input['encrypt_key']) != 8)) {
            ...
            // 填充原字符
            $keySource = strtoupper($input['encrypt_key'] . '00000000');
